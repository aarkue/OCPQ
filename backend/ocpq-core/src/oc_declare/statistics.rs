...
/// The events of type `etype` a binding's object filters select.
///
/// Mirrors `process_mining`'s internal `target_events_for_binding` with no reverse-index view,
/// which is `pub(crate)` upstream. `Any` is a union (an event referencing several of the objects
/// still counts once); `All` starts from the first object and keeps only events referencing the
/// rest; an empty filter list means every event of the type.

...
/// An evaluation result is a registry handle rather than a value on the wire: the situations of a
/// large tree run to millions of rows, and the frontend only ever reads a page at a time. Storing
/// it means `eval_results_page` can be an ordinary binding instead of transport-specific state.
...
/// Evaluation of whole trees against a real (tiny) OCEL.
///
/// The log below is the fixture every test here shares:
///
/// | object | type  | o2o                |
/// |--------|-------|--------------------|
/// | o1     | order | contains -> i1     |
/// | o2     | order | --                 |
/// | i1     | item  | --                 |
///
/// | event | type   | day | e2o                                 |
/// |-------|--------|-----|-------------------------------------|
/// | e1    | place  | 1   | order -> o1  (amount 100)           |
/// | e2    | place  | 2   | order -> o2  (amount 5)             |
/// | e3    | pay    | 3   | order -> o1                         |
/// | e4    | ship   | 4   | order -> o1, item -> i1             |
/// | e5    | ship   | 5   | order -> o1                         |
/// | e6    | cancel | 6   | order -> o2                         |
#[cfg(test)]
mod evaluation_tests {
    ...
    fn a_predicate_as_a_constraint_reports_a_violation_where_as_a_filter_it_removes_the_binding() {
        // Same predicate (place.amount >= 10), same two candidate bindings: (o1,e1) with 100 and
        // (o2,e2) with 5. As a `filter` the failing binding disappears; as a `constraint` it stays
        // and is reported violated. Confusing the two is silent, so pin both halves together.
    ...
    /// The name of the edge is what joins a `NumChilds` constraint to the child results; an edge
    /// name that no constraint mentions leaves the count unknown, which counts as a violation

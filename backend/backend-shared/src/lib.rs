...
use ocpq_core::{
    binding_box::EvaluateBoxTreeResult,
    table_export::{export_bindings_to_writer, TableExportOptions},
};
use process_mining::{
    bindings::{self, RegistryItem, RegistryItemKind},
    core::{
        event_data::{
            case_centric::xes::{XESOuterLogData, XESParsingTraceStream},
            object_centric::linked_ocel::LinkedOCELAccess,
        },
...
/// Invalidates `id`'s cached `{id}__as__{Kind}` conversions by bumping its generation, so a later
/// query can't serve a stale conversion. Call this on every path that overwrites an existing id.
...
pub fn unload_artifact<B: Backend>(backend: &B, id: String) -> Result<(), String> {
    ...
    // The whole entry, not just the label: leftover role/provenance made a later reimport under
    // the same id look derived from the unloaded one's sources.
    st.meta.remove(&id);
    emit_artifacts_changed(backend);
    Ok(())
}
...
fn reconcile_order(order_lock: &RwLock<Vec<String>>, present: &HashSet<String>) -> Vec<String> {
    let mut order = order_lock.write().unwrap_or_else(|e| e.into_inner());
...
/// Move the object stored under `from` to `to`, so a caller can build a result under a scratch id
/// and adopt it only once the work that produced it succeeded.
pub fn rename_object<B: Backend>(backend: &B, from: &str, to: &str) -> Result<(), String> {
    ...
    let to_prefix = format!("{to}__as__");
    {
        ...
        // Both ids' cached `__as__` conversions still validate as fresh against the unchanged
        // generation, but now describe content neither id holds; drop them.
...
/// Export an object straight to a filesystem path. Not covered by [`export_object`]: the OCEL 2.0
/// bundled format's uncompressed form is a directory, which has no byte stream at all.
...
/// Render the situations of one node of a stored evaluation as a CSV/XLSX file. Takes plain ids
/// rather than going through [`execute_binding`]: the result is a file, not a registry/JSON value.
pub fn export_bindings_table_file<B: Backend>(
    backend: &B,
    ocel_id: &str,
    eval_id: &str,
    node_index: usize,
    ...
    let items = backend
        ...
        .read()
        .map_err(|e| e.to_string())?;
    let ocel = match items.get(ocel_id) {
        Some(RegistryItem::SlimLinkedOCEL(ocel)) => ocel,
        Some(_) => return Err(format!("{ocel_id} is not a SlimLinkedOCEL")),
        None => return Err("OCEL not found".to_string()),
    };
    let eval = items
        .get(eval_id)
        .and_then(|i| i.as_custom::<EvaluateBoxTreeResult>())
        .ok_or_else(|| "Evaluation not found".to_string())?;
    // Same check `app_bindings::query::same_ocel` applies: a binding's indices are only meaningful
    // for the log they were resolved against, not any log currently stored under `ocel_id`.
    ...
    export_bindings_to_writer(ocel, node_res, &mut cursor, options).map_err(|e| e.to_string())?;
...
pub fn execute_binding<B: Backend>(
    ...
    // A registry-storing binding takes an `output_id`, so a caller-chosen name lands under that id
    // directly instead of needing a re-key afterwards.
    ...
    if let (true, Ok(bytes)) = (stores_result, &result) {
        ...
        if let Ok(id) = serde_json::from_slice::<String>(bytes) {
            if st.contains_key(&id) {
                // A named output replaces whatever that id held, like any import; without this its
                // stale `{id}__as__{Kind}` conversions would still validate as fresh.
                mark_replaced(st, &id);
                // Keep the existing role (`Primary` if fresh): a named output stays listed, unlike
                // the hidden machine-minted `__as__` conversions above.
                st.meta.set(
                    &id,
                    ItemMeta {
...
mod convert_dispatch_tests {
    ...
    fn schemas() -> Vec<(String, serde_json::Value)> {
        vec![(
            "locel".into(),
            json!({ "x-registry-ref": "SlimLinkedOCEL" }),
        )]
    }
...
mod artifact_store_tests {
    ...
    /// `output_name` names the stored result; it is not a hiding flag, so the object stays listed
    /// under that id rather than being stamped `Result` and hidden.
    #[test]
...
#[cfg(test)]
mod export_bindings_table_tests {
    ...
    const OCEL_JSON: &str = r#"{
        ...
        "objects": [{ "id": "o1", "type": "order", "relationships": [], "attributes": [] }],
        "events": [
            { "id": "e1", "type": "place", "time": "2024-01-01T00:00:00Z", "attributes": [],
              "relationships": [{ "objectId": "o1", "qualifier": "order" }] }
        ]
    }"#;

    /// A stored `SlimLinkedOCEL` (`ocel_id`) plus a stored evaluation of one node binding every
    /// order to its `place` event (`eval_id`).
    fn seeded_backend() -> StubBackend {
        let b = backend();
        ...
        let node = BindingBoxTreeNode::Box(
            BindingBox {
                new_object_vars: Map::from([(
                    ObjectVariable(0),
                    HashSet::from(["order".to_string()]),
                )]),
                new_event_vars: Map::from([(
                    EventVariable(0),
        ...
        let eval = evaluate_box_tree(tree, &ocel, false).expect("evaluation succeeds");
        b.get_state().add("ocel", ocel);
        b.get_state()
            .items
            .write()
            .unwrap()
            .insert("eval".to_string(), RegistryItem::custom(eval));
        b
    }

    #[test]
    fn csv_export_contains_the_bound_object_id() {
        let b = seeded_backend();
        let bytes =
            export_bindings_table_file(&b, "ocel", "eval", 0, &TableExportOptions::default())
    ...
    #[test]
    fn missing_ocel_is_an_error() {
        let b = seeded_backend();
        let err = export_bindings_table_file(
            &b,
            "no-such-ocel",
            "eval",
            0,
    ...
    #[test]
    fn out_of_range_node_index_is_an_error() {
        let b = seeded_backend();
        let err = export_bindings_table_file(&b, "ocel", "eval", 99, &TableExportOptions::default())
            .unwrap_err();

//! OCED path-schema analysis.
//!
//! `process_mining` registers path-schema bindings of its own, but they are not the same
//! functions: OCPQ's carry per-type entity counts, equivalence-class counts and the per-schema
//! detail view (connections, throughput, durations) the viewer is built around, and upstream has
//! no equivalent of `schema_detail` at all. So these wrap `ocpq-core`'s, and the frontend calls
//! them directly rather than reshaping upstream's output.

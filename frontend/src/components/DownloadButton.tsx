...
export function DownloadButton({ fileName, value }: { fileName: string, value: string|Blob }) {
  ...
  return <Button className="h-7" variant="ghost" size="icon" title="Download" onClick={() => {
    backend["download-blob"](typeof value === "string" ? new Blob([value], { type: "text/plain" }) : value, fileName );

...
import { type ConnectionConfig, connectionFormSchema } from "@/types/hpc-backend";

const SAVED_AUTH_LOCAL_STORAGE_KEY = "saved-auth";
const ConnectionConfigForm = React.forwardRef<
	{ getConfig: () => ConnectionConfig },
	{ disabled?: boolean; onSubmit: (config: ConnectionConfig) => any }
>(({ disabled }, ref) => {
				...
				<div>
						...
						render={({ field }) => (
							<FormItem>
								<FormControl>
									<Tabs
										...
										onValueChange={(newMode) => {
											if (newMode === "password-mfa") {
												const newVal: z.infer<typeof connectionFormSchema>["auth"] = {
													mode: "password-mfa",
													password: "",
													mfaCode: "",
												};
									...
									>
										<TabsList>
											<TabsTrigger disabled={disabled} value="password-mfa">
												Password + MFA
											</TabsTrigger>
											<TabsTrigger disabled={disabled} value="ssh-key">
												Private SSH Keyfile
											</TabsTrigger>
										</TabsList>
										<div className="text-left ml-4">
											...
											<TabsContent value="ssh-key">
												Login using a private SSH key, optionally protected with an additional
												passcode.
												<FormField
													disabled={disabled}
													control={form.control}
					...
					<Label className="flex items-center mb-2 w-fit">
						<Checkbox
							...
							onCheckedChange={(c) => setSaveLoginInfo(c === true)}
						/>
						Save login info
					</Label>
				</div>

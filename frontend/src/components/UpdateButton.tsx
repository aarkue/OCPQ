...
export function UpdateButton() {
    ...
    if (backend['check-for-updates'] == null) {
        return null;
    }
    const [updateState, setUpdateState] = useState<UpdateInfo | null>(null);
    const [status, setStatus] = useState<{ contentLenght?: number, downloaded: number, state: "initial" | "downloading" | "downloaded" | "installing" | "installed" | "restarting", error?: string }>({ downloaded: 0, state: "initial" });
    useEffect(() => {
    ...
    return <div className="border p-1 rounded-md shadow-inner shadow-sky-200/50">
        <h2 className="text-lg font-bold mt-1 text-sky-600">Update available</h2>
        <p className="mb-1">Version {updateState.version}</p>
       {status.error == undefined && <>
        {(status.state === "initial" || status.state === "downloading") && <Button disabled={status.state === "downloading"} onClick={() => {
            ...
            updateState?.download((ev) => {
                if (ev.event === "Started") {
                    setStatus((status) => ({ ...status, contentLenght: ev.data.contentLength, state: "downloading" }));

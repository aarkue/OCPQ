...
function makeFake(options: FakeOptions = {}): Fake {
	...
	const backend: BackendContext = {
		...
		exportBindingsTable: async () => new Uint8Array(),
...
describe("ocel/info", () => {
	// A caller-named output must keep the default `Primary` role, not `Result`, since
	// `get_objects_with_type` hides `Result`-tagged items from the listing.
...
describe("ocel/upload-from-xes", () => {
	// The engine reads `xes` into the object-centric kinds itself, so this is one import with no
	// scratch handle or intermediates to leak.
...
describe("data-extraction/execute", () => {
	...
	// A failed run must not overwrite the loaded log; only the scratch id gets dropped.
...
describe("optional provider keys", () => {
	...
	// A picked `.xes` goes through the same OCEL-kind path as every other file the picker offers.

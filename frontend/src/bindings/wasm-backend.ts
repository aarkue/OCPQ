...
/**
 * Where wasm-pack's output is served from. Deliberately not a literal in the `import()` below:
 * the package is not built yet, and a literal specifier would make both tsc and the bundler try
 * to resolve it at build time.
 */
export const WASM_MODULE_URL = "/wasm/ocpq_wasm.js";

/**
 * In-browser backend: the engine runs as a wasm module on the page.
 *
 * Every call is synchronous once the module is initialised; the promises exist so this fits the
 * same {@link BackendContext} as the http and tauri transports.
 */
export function createWasmBackend(moduleUrl: string = WASM_MODULE_URL): BackendContext {
	...
	const load = (): Promise<WasmModule> => {
		if (modP === undefined) {
			// The engine calls `wasmSpace.emit` for every event it emits, so the host must provide
			// it before the module runs.

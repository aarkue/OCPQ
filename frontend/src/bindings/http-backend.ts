...
/** Remote backend: talks to the engine running in the `web-server` (axum) process over HTTP.
 *  `base` is "/api" when axum also serves the UI, or a full origin otherwise. */
export function createHttpBackend(base = "/api"): BackendContext {
	const root = base.replace(/\/$/, "");

	// One shared SSE connection, opened lazily on first registerListener; tracked subs re-attach on reconnect.
	...
	return {
		...
		async exportBindingsTable(ocelId, evalId, nodeIndex, options) {
			const res = await fetch(`${root}/export-bindings-table`, {
				method: "POST",
				headers: { "content-type": "application/json" },
				body: JSON.stringify({
					ocel_id: ocelId,

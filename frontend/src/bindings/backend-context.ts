 ...
 * (http fetch / tauri invoke / in-browser wasm). Data stays in the engine; only handles,
 * metadata, file bytes, and small results cross this boundary.
 *
 * Everything is async so http and tauri fit without special-casing; wasm wraps its
...
export interface BackendContext {
	...
	/**
	 * Move the object at `from` onto `to`, replacing whatever `to` held; fails if `from` is absent.
	 * Lets a caller build a result under a scratch id and adopt it only once the work succeeded.
	 */
	...
	/**
	 * Subscribe to a backend-emitted engine event (`import-finished`, `objects-changed`, ...).
	...
	/**
	 * Desktop-only: write the export straight to `path`. `format` is passed explicitly because a
	 * path need not carry it, and it keeps a large log out of the IPC boundary.
	 */
	 ...
	 * Server-side convenience, so http only.
	 */

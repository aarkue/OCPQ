...
import type { CustomEdgeType } from "./oc-declare-flow-types";
...
export async function applyLayoutToNodes<N extends Record<string, unknown>>(
	...
	const graph = {
		...
		edges: edges.map((e) => ({
			id: e.id,
			sources: [
				e.data?.type.includes("rev") ? (e.targetHandle ?? e.target) : (e.sourceHandle ?? e.source),
			],
			targets: [
				e.data?.type.includes("rev") ? (e.sourceHandle ?? e.source) : (e.targetHandle ?? e.target),
			],
			properties: {},
			layoutOptions: {
				"org.eclipse.elk.stress.desiredEdgeLength":
					120 +
					6 *
						((e.data?.objectTypes.all.length ? 10 * e.data?.objectTypes.all.length + 10 : 0) +
							(e.data?.objectTypes.any.length ? 10 * e.data?.objectTypes.any.length + 10 : 0) +

...
export function getMarkersForEdge(
	edgeType: EdgeType,
	id?: string,
): {
	markerStart: string;
...
export default function OCDeclareFlowEditor({
	initialFlowJson,
	onChange,
	onInit,
	name,
	...
	name: string;
	initialFlowJson?: ReactFlowJsonObject<ActivityNodeType, CustomEdgeType>;
	onInit?: (ref: ReactFlowInstance<ActivityNodeType, CustomEdgeType>) => unknown;
	onChange: (json: ReactFlowJsonObject<ActivityNodeType, CustomEdgeType>) => unknown;
}) {
	...
	const onConnect = useCallback<OnConnect>(
		(connection) => {
			...
			const sourceRelatedTypes =
				(sourceIsObject ? ocelInfo?.o2o_types : ocelInfo?.e2o_types)?.[source.data.type] ?? {};
			const targetRelatedTypes =
				(targetIsObject ? ocelInfo?.o2o_types : ocelInfo?.e2o_types)?.[target.data.type] ?? {};
			const commonTypes = Object.keys(sourceRelatedTypes).filter(
				(k) =>
					targetRelatedTypes[k] !== undefined &&
					targetRelatedTypes[k][0] > 0 &&
					sourceRelatedTypes[k][0] > 0,
			);

			let objectTypes: OCDeclareArcLabel = {
				all: commonTypes.map((t) => ({ type: "Simple", object_type: t })),
				any: [],
				each: [],
			};
			if (source.data.isObject && !target.data.isObject) {
				objectTypes = {
					each: [],
					any: [{ type: "Simple", object_type: source.data.type }],
					all: [],
				};
			} else if (target.data.isObject && !source.data.isObject) {
				objectTypes = {
					each: [],
					any: [{ type: "Simple", object_type: target.data.type }],
					all: [],
				};
			} else if (target.data.isObject && source.data.isObject) {
				objectTypes = {
					...
					any: [
						{
							type: "O2O",
							first: source.data.type,
							second: target.data.type,
							reversed: false,
						},
					],
					all: [],
		...
		[ocelInfo?.e2o_types, ocelInfo?.o2o_types],
	);

	const [edges, _setEdges, onEdgesChange] = useEdgesState<CustomEdgeType>([]);
	...
	const createNewNodeAtPosition = useCallback(
		(
			{ x, y }: { x: number; y: number },
			activity: string | null = null,
			isObject: "init" | "exit" | undefined = undefined,
		) => {
			const act = activity || ocelInfo?.event_types?.[0]?.name || "new activity";
			flowRef.current?.addNodes({
				id: uuidv4(),
				type: "activity",
				data: { type: act, isObject },
	...
	useEffect(() => {
		...
		function addPastedData(nodes: ActivityNodeType[], edges: CustomEdgeType[]) {
		...
		function pasteListener(ev: ClipboardEvent) {
			...
			if (ev.clipboardData != null) {
				let pastedNodesAndEdges = ev.clipboardData.getData("application/json+oc-declare-flow");
		...
		async function keyPressListener(ev: KeyboardEvent) {
			...
			if (ev.altKey && ev.key === "n") {
				...
				ev.preventDefault();
				ev.stopPropagation();
				flowRef.current!.setNodes((nodes) => nodes.map((n) => ({ ...n, selected: true })));
				flowRef.current!.setEdges((edges) => edges.map((e) => ({ ...e, selected: true })));
				return false;
			}
		}
	...
	}, [autoLayout, createNewNodeAtPosition]);
	...
	return (
		<>
			<ContextMenu>
				...
				<ContextMenuContent>
					<ContextMenuSub>
						<ContextMenuSubTrigger>Add Activity</ContextMenuSubTrigger>
					...
					<ContextMenuSub>
						<ContextMenuSubTrigger>Add Object Init</ContextMenuSubTrigger>
						<ContextMenuSubContent className="overflow-auto max-h-48">
					...
					<ContextMenuSub>
						<ContextMenuSubTrigger>Add Object Exit</ContextMenuSubTrigger>
						<ContextMenuSubContent className="overflow-auto max-h-48">
							{ocelInfo?.object_types?.map((ot) => (
								<ContextMenuItem
									key={ot.name}
			...
			<div className="outer-flow w-full h-full select-none">
				<ReactFlow
					...
					onInit={(i) => {
						if (
							initialFlowJson &&
							"nodes" in initialFlowJson &&
							"edges" in initialFlowJson &&
							"viewport" in initialFlowJson
						) {
					...
					onSelectionChange={(sel) => {
						...
						for (const n of sel.nodes) {
							for (const n2 of sel.nodes) {
								const edgesToAdd = flowRef.current
									?.getEdges()
									.filter(
										(e) =>
											e.source === n.id &&
											e.target === n2.id &&
											sel.edges.find((e2) => e2.id === e.id) == null,
									)
									.map((e) => e.id);
								if (edgesToAdd) {
						...
						if (addedEdges.size > 0) {
							flowRef.current?.setEdges((edges) =>
								[...edges].map((e) => ({
									...e,
									selected: e.selected || addedEdges.has(e.id),
								})),
							);
						}
				...
				>
					...
					<Panel position="top-right" className="flex gap-x-1 hide-in-image">
						<AlertHelper
							initialData={{ format: "text" as "text" | "json" }}
							trigger={
								<Button title="Export model" variant="outline" size="icon">
									<LuShare />
								</Button>
							}
							mode="normal"
							title="Export OC-DECLARE"
							content={({ data, setData }) => {
								const result = useQuery({
									queryKey: ["export-oc-declare", data.format, edges],
									queryFn: async () => {
										if (data.format === "text") {
											return await backend["oc-declare/template-string"](
												flowRef
													.current!.getEdges()
													.map((e) => flowEdgeToOCDECLARE(e, flowRef.current!)),
											);
										}
								...
								return (
									<>
										<div className="mb-2">
											Export the current OC-DECLARE model as JSON or as text representation.
										</div>
										<div className="flex flex-col gap-y-2">
											<div className="mr-2 flex flex-col  items-start  gap-2">
												...
												<ToggleGroup type="single" value={data.format}>
													<ToggleGroupItem
														value="text"
														onClick={() => setData({ ...data, format: "text" })}
													>
														Text
													</ToggleGroupItem>
													<ToggleGroupItem
														value="json"
														onClick={() => setData({ ...data, format: "json" })}
													>
														JSON
													</ToggleGroupItem>
												</ToggleGroup>
											...
											<div>
												<pre className="h-96 w-full overflow-auto rounded-md border p-2 bg-muted">
													<code>
														{result.data
															? result.data
															: result.isLoading
												...
												<div className="flex justify-end">
													<ClipboardButton
														value={result.data ?? ""}
														name="OC-DECLARE Model"
														hideValueInToast
													/>
													<DownloadButton
														value={result.data ?? ""}
														fileName={`oc-declare.${data.format === "json" ? "json" : "txt"}`}
													/>
												</div>
											</div>
										</div>
...
*/}
						<div className="flex flex-row-reverse items-center gap-x-1">
							<Button
								onClick={async () => {
									...
									const edges =
										selectedEdges.length > 0 ? selectedEdges : flowRef.current!.getEdges();
									const edgeIDs = edges.map((e) => e.id);
									const edgesConverted = edges.map((e) => flowEdgeToOCDECLARE(e, flowRef.current!));

									const res = await toast.promise(
										backend["ocel/evaluate-oc-declare-arcs"](edgesConverted),
										{
											loading: "Evaluating...",
									...
									for (let i = 0; i < edgeIDs.length; i++) {
										flowRef.current?.updateEdgeData(edgeIDs[i], {
											violationInfo: { violationPercentage: 100 * res[i] },
										});
									}
								}}
							>
							...
							{edges.find((e) => e.data?.violationInfo) !== undefined && (
								<Button
									size="icon"
									variant="outline"
									...
									className=""
									onClick={async () => {
										flowRef!.current?.setEdges((eds) =>
											eds.map((e) => ({
												...e,
												data: { ...e.data!, violationInfo: undefined },
											})),
										);
									}}
								>
						...
						<Button
							...
							onClick={(ev) => {
								...
								const viewPort = document.querySelector(".outer-flow") as HTMLElement;

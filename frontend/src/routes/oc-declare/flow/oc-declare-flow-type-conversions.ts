export function translateArcInfo(
	data: CustomEdgeType["data"],
): [OCDeclareArcType, [number | null, number | null]] {
	switch (data!.type) {
		case "ef":
		...
		case "df":
			return ["DF", data?.cardinality ?? [1, null]];
		case "df-rev":
			return ["DP", data?.cardinality ?? [1, null]];
		case "ndf":
			return ["DF", [0, 0]];
		case "ndf-rev":
			return ["DP", [0, 0]];
...
export function flowEdgeToOCDECLARE(
	e: CustomEdgeType,
	flow: ReactFlowInstance<ActivityNodeType, CustomEdgeType>,
): OCDeclareArc {
	const [arc_type, counts] = translateArcInfo(e.data!);
	return {
		from:
			flow.getNode(e.source)!.data.isObject === "init"
				? `<init> ${flow.getNode(e.source)!.data.type}`
				: flow.getNode(e.source)!.data.isObject === "exit"
					? `<exit> ${flow.getNode(e.source)!.data.type}`
					: flow.getNode(e.source)!.data.type,
		to:
			flow.getNode(e.target)!.data.isObject === "init"
				? `<init> ${flow.getNode(e.target)!.data.type}`
				: flow.getNode(e.target)!.data.isObject === "exit"
					? `<exit> ${flow.getNode(e.target)!.data.type}`
					: flow.getNode(e.target)!.data.type,
		arc_type,
		counts,
...
import type { OCDeclareNode } from "../types/OCDeclareNode";
import { applyLayoutToNodes } from "./automatic-layout";
import { getMarkersForEdge } from "./OCDeclareFlowEditor";
import type { ActivityNodeType, CustomEdgeType, EdgeType } from "./oc-declare-flow-types";
// import { ObjectTypeAssociation } from "crates/shared/bindings/ObjectTypeAssociation";
export async function addArcsToFlow(
	discoverdArcs: OCDeclareArc[],
	flow: ReactFlowInstance<ActivityNodeType, CustomEdgeType>,
) {
	...
	for (const arc of discoverdArcs) {
		...
		edges.push({
			...
			source: sourceID,
			target: targetID,
			data: { type: edgeType, objectTypes: arc.label, cardinality: arc.counts },
			...getMarkersForEdge(edgeType, edgeID),
		});
		// }
		// }
	}
...
function lookupIDOrCreateNode(
	node: OCDeclareNode,
	nodeIDMap: Record<string, string>,
	nodes: ActivityNodeType[],
): string {
	let isObject: ActivityNodeType["data"]["isObject"];
	...
	if (nodeIDMap[node] === undefined) {
		...
		nodes.push({
			...
			type: "activity",
			position: { x: 0, y: 0 },
			data: { isObject, type: node.replace("<init> ", "").replace("<exit> ", "") },
		});
		nodeIDMap[node] = id;
		return id;
	}

...
export function getParentNodeID(nodeID: string, edges: Edge<EventTypeLinkData>[]) {
...
export function getParentsNodeIDs(nodeID: string, edges: Edge<EventTypeLinkData>[]): string[] {
	for (const edge of edges) {
		if (edge.target === nodeID) {
...
function getChildrenNodeIDsRec(nodeID: string, edges: Edge<EventTypeLinkData>[]): string[] {
	const children = getChildrenNodeIDs(nodeID, edges);
	return [nodeID, ...children.flatMap((c) => getChildrenNodeIDsRec(c, edges))];
}
...
export function evaluateConstraints(
	...
	for (const root of roots) {
		const nodesOrder = getChildrenNodeIDsRec(root.id, edges).map((nid) => nodeIDMap.get(nid)!);
		...
		tree.nodes = nodesOrder.map((node) => {
			...
			for (const c of children) {
				...
				if (name != null) {
					tree.edgeNames.push([[nodesIndexMap.get(node.id)!, nodesIndexMap.get(c)!], name]);
				}
			...
			if (node.data.type === "or" && children.length === 2) {
				const [c1, c2] = children;
				return { OR: [nodesIndexMap.get(c1)!, nodesIndexMap.get(c2)!] };
			}
			if (node.data.type === "not" && children.length === 1) {
...
\begin{schema}{\boxFunc(v_{${i}}) \text{ with } \constrFunc{v_{${i}}}}
      ${Object.entries(box.newObjectVars)
				...
				.map(
					([key, val]) =>
						`\\texttt{e${Number.parseInt(key, 10) + 1}}: \\textsc{Event} (${val
							.map((v) => `\\texttt{${v}}`)
							.join(", ")})\\\\`,
				)
				.join("\n      ")}
      \where ${[...box.filters, ...box.sizeFilters].map((f) => predicateToLaTeX(f)).join("\\\\\n")}
...
function predicateToLaTeX<T extends Filter | SizeFilter | Constraint>(value: T) {
	...
	switch (value.type) {
		...
		case "TimeBetweenEvents":
			return String.raw`\mathrm{TBE}(\texttt{e${
				...
				value.min_seconds ?? Number.NEGATIVE_INFINITY,
				"$\\infty$",
				"$-\\infty$",
			)}},\text{${formatSeconds(value.max_seconds ?? Number.POSITIVE_INFINITY, "$\\infty$", "$-\\infty$")}})`;
		case "NumChilds":
		...
		case "NumChildsProj":
			...
			return String.raw`\mathrm{CBPS}(\texttt{${
				value.child_name
			}},\texttt{${varName(value.var_name)}},${value.min ?? 0},${value.max ?? "\\infty"})`;
		// if (value.min !== null && value.min === value.max) {
		...
		case "ObjectAttributeValueFilter":
			{
				...
				const timePart = String.raw`\texttt{${
...
function _bindingTreeToTikzTree(tree: BindingBoxTree) {
	let s = String.raw`
\begin{tikzpicture}
...
`;
	for (const [[from, to], name] of edgeNames) {
		s += String.raw`  v${from} -> ["\texttt{${name}}"${i % 2 === 0 ? ",swap" : ""}] v${to},`;
		s += "\n";
		i++;
	}
	return (

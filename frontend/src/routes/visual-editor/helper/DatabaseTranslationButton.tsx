...
export default function DatabaseTranslationButton({ instance }: { instance: ReactFlowInstance<Node<EventTypeNodeData | GateNodeData>, Edge<EventTypeLinkData>>; }) {
  ...
  return <AlertHelper trigger={<Button
    variant="outline"
    size="icon"
  ...
  >
    ...
    content={({ data, setData }) => {
      return <div>
        ...
        <div className="grid grid-cols-2 items-center font-semibold mb-2">SQL Dialect 
        <Select value={data.dialect} onValueChange={(x) => setData({...data, dialect: x as any})}>
        ...
        <div className="ml-3 mt-1">
          ...
          <div className="text-xs flex flex-wrap gap-1 mt-1 ml-2">
            <Button size="sm" variant="outline" onClick={() => setData({ ...data, eventMapping: getTranslationValue(ocelInfo.event_types.map(t => t.name)), objectMapping: getTranslationValue(ocelInfo.object_types.map(t => t.name)) })}>
              Aa bb<TbArrowRight />Aa bb
            </Button>
            <Button size="sm" variant="outline" onClick={() => setData({ ...data, eventMapping: getTranslationValue(ocelInfo.event_types.map(t => t.name), ' ', true), objectMapping: getTranslationValue(ocelInfo.object_types.map(t => t.name), ' ', true) })}>
              Aa bb<TbArrowRight />Aa Bb
            </Button>
            <Button size="sm" variant="outline" onClick={() => setData({ ...data, eventMapping: getTranslationValue(ocelInfo.event_types.map(t => t.name), ' ', false, true), objectMapping: getTranslationValue(ocelInfo.object_types.map(t => t.name), ' ', false, true) })}>
              Aa bb<TbArrowRight />aa bb
            </Button>
            <Button size="sm" variant="outline" onClick={() => setData({ ...data, eventMapping: getTranslationValue(ocelInfo.event_types.map(t => t.name), '', true), objectMapping: getTranslationValue(ocelInfo.object_types.map(t => t.name), '', true) })}>
              Aa bb<TbArrowRight />AaBb
            </Button>
            <Button size="sm" variant="outline" onClick={() => setData({ ...data, eventMapping: getTranslationValue(ocelInfo.event_types.map(t => t.name), ''), objectMapping: getTranslationValue(ocelInfo.object_types.map(t => t.name), '') })}>
              Aa bb<TbArrowRight />Aabb
            </Button>
            ...
            <Button size="sm" variant="outline" onClick={() => setData({ ...data, eventMapping: getTranslationValue(ocelInfo.event_types.map(t => t.name), '_', false), objectMapping: getTranslationValue(ocelInfo.object_types.map(t => t.name), '_', false) })}>
              Aa bb<TbArrowRight />Aa_bb
            </Button>
            <Button size="sm" variant="outline" onClick={() => setData({ ...data, eventMapping: getTranslationValue(ocelInfo.event_types.map(t => t.name), '_', true), objectMapping: getTranslationValue(ocelInfo.object_types.map(t => t.name), '_', true) })}>
              Aa bb<TbArrowRight />Aa_Bb
            </Button>
            <Button size="sm" variant="outline" onClick={() => setData({ ...data, eventMapping: getTranslationValue(ocelInfo.event_types.map(t => t.name), '_', false, true), objectMapping: getTranslationValue(ocelInfo.object_types.map(t => t.name), '_', false, true) })}>
              Aa bb<TbArrowRight />aa_bb
            </Button>
          </div>
        <Accordion type="multiple" defaultValue={result === undefined ? ["objects", "events"] : []}>
        ...
        {result !== undefined && <div className="mt-4 relative">
          <Button size="sm" title="Copy to clipboard" className="absolute top-0 right-0" onClick={() => {
            navigator.clipboard.writeText(result);
            toast.success("Copied Query");
          }}><LuClipboard className="size-4" /></Button>
          <h3 className="font-semibold text-base mb-2">Generated Query</h3>
          <pre className="bg-gray-100 p-2 rounded-md overflow-x-auto"><code>{result}</code></pre>
        </div>}
      </div>
    }}

...
export default function OcelGraphViewer({
	...
	const handleSetGraphData = useCallback((gd: GraphData | undefined) => {
		prevGraphDataRef.current = undefined;
		const _link = graphRef.current!.d3Force("link")!;
	...
	return (
		<div className="text-lg text-left w-full h-full">
			<div className="flex h-full w-full items-start gap-x-1">
				...
				/>
				<div
					className="border-2 border-dashed w-full h-full overflow-hidden relative"
					ref={containerRef}
					tabIndex={-1}
				>
					<Button
						disabled={prevGraphDataRef.current === undefined}
					...
					<Button
						...
						onClick={(ev) => {
							const canvas = ev.currentTarget.parentElement?.querySelector("canvas");
							if (canvas != null) {
								canvas.toBlob((blob) => {
									if (blob !== null) {
							...
							{({ height, width }) => (
								<ForceGraph2D
									...
									nodeCanvasObject={(node, ctx) => {
										...
										let width = 40;
										let height = 40;
										const fillStyle = isFirstNode ? node.color : `${node.color}74`;
										...
										if ("time" in node) {
											...
											ctx.beginPath();
											ctx.fillStyle = "white";
											ctx.roundRect(node.x - width / 2, node.y - height / 2, width, height, 0.2);
											ctx.fill();
									...
									onNodeClick={async (node) => {
										...
										void toast
											.promise(
												backend["ocel/graph"]({
													root: node.id,
													rootIsObject: !("time" in node),
													maxDistance: 1,
													relsSizeIgnoreThreshold: 100,
													spanningTree: false,
												}),
												{
...
function GraphOptions({
	...
	const applyGraph = useCallback(
		(graphOptions: OCELGraphOptions = optionsRef.current) => {
			setLoading(true);
			void toast
		...
		[backend, setGraphData],
	);

	useEffect(() => {
		if (initialGrapOptions?.id !== undefined && initialGrapOptions?.type !== undefined) {
			...
			applyGraph(updatedOptions);
		}
	}, [initialGrapOptions?.id, initialGrapOptions?.type, setOptions, applyGraph]);
	return (
			...
			<div className="h-fit">
				...
				<div className="flex flex-col gap-y-1 mb-4">
					<div className="flex gap-x-1 items-center">
						<Label
							className="w-[9ch] cursor-help"
							title="Entity type (i.e., Object/Event) of the Object/Event to query"
						>
							Root Type
						</Label>
						<ToggleGroup
					...
					<div className="flex gap-x-1 items-center">
						<Label className="w-[12ch] cursor-help" title="ID of the Object/Event to query">
							Root ID
						</Label>
						...
						<Input
							...
							type="text"
							value={options.root}
							onChange={(ev) => setOptions({ ...options, root: ev.currentTarget.value })}
						/>
					</div>
					<div className="flex gap-x-1 items-center">
						<Label
							className="w-[12ch] cursor-help"
							title="Maximum distance (i.e., number of hops along E2O/O2O relationship edges) to view"
						>
							Max. Distance
						</Label>
						<Input
					...
					<div className="flex gap-x-1 items-center">
						<Label
							className="w-[12ch] cursor-help"
							title="Maximum number of neighbors (i.e., entities involved through E2O/O2O) to recursively expand. This option prevents polluting the graph with too many nodes."
						>
							Max. Neighbors
						</Label>
						<Input
					...
					<Button size="lg" disabled={loading} onClick={() => applyGraph()}>
						Apply
					</Button>
				</div>
				<p className="text-xs mt-2 border border-blue-200 px-2 py-1 rounded-sm bg-blue-50/80 text-wrap max-w-sm">
					<span className="font-bold inline-flex items-center gap-x-1 text-sm">
						<PiInfoBold className="inline text-blue-600" size={18} /> Legend & Hints
					</span>
					<br />
					Events are shown as as squares and objects as circles in the graph. Hovering over an edge
					between two nodes reveals the direction of the relation as well as the qualifier.
					<br />
					Click on an node to expand its relations. Right-click a node to copy its ID to the
					clipboard.
				</p>
			</div>
		</div>

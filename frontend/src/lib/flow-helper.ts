...
export function isEditorElementTarget(el: HTMLElement | EventTarget | null,isInitial = true): boolean {
  ...
  return (
    ...
    (el !== null && "className" in el && el.className !== undefined && typeof el.className.includes === 'function' && (el.className?.includes("react-flow") || isEditorElementTarget(el.parentElement,false)))
  );
}
